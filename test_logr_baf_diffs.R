...
parser$add_argument("--lift_hg38",        default = FALSE, help = "Boolean. If the BAF and LogR files provided are hg38, liftover to hg19")
parser$add_argument("--chain_file_path",  default = NULL, help = "Liftover file from hg38 to hg19, needed only if lift_hg38 is TRUE")
parser$add_argument("--results_dir",      help = "Directory to store results")

args <-parser$parse_args()

# Functions----------------------------------------------------------------------------------
# Function to liftover ASCAT inputs generated by ascat.generateHTS from hg38 to hg19
liftover_ascat_input <- function(ascat_in_dt, chain_file_path){
  
  # Load chain file
  chain <- import.chain(chain_file_path)

  # Get input into GRanges
  in_colnames <- colnames(ascat_in_dt)
  ascat_in_gr <- GRanges(seqnames = ascat_in_dt$chrom,
                         ranges = IRanges(start = ascat_in_dt$pos,
                                          end = ascat_in_dt$pos),
                         metadata = ascat_in_dt[, get(in_colnames[3])])

  ascat_out_dt <- as.data.table(unlist(liftOver(ascat_in_gr, chain)))
  # Keep columns of interest, rename and output
  gr_names <- c('seqnames', 'start', 'metadata')
  ascat_out_dt <- ascat_out_dt[, ..gr_names]
  setnames(ascat_out_dt, gr_names, in_colnames)
...
#Lift over if hg38 input
if(args$lift_hg38){
  assertthat::assert_that(!is.null(args$chain_file_path), msg = "Path to chain file needs to be provided if running liftover")
  ascat_baf_filt <- liftover_ascat_input(ascat_baf_filt, args$chain_file_path)
  ascat_logr_filt <- liftover_ascat_input(ascat_logr_filt, args$chain_file_path)
}
